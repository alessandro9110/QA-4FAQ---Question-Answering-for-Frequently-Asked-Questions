...
if __name__ == '__main__':
    ...
    #ANSWER TRAINING
    original_QA_train = loadSentences("Armillotta/tokenizer/tokenizerQuestionAnswer.txt")
    QA_train = loadSentences("Armillotta/tokenizer/tokenizerQuestionAnswer.txt")
    ...
    checkpoint = ModelCheckpoint("Armillotta/keras_models/Q_QA_Conv2D_MaxPool.h5", monitor='val_acc', verbose=1, save_best_only=True, mode='max')
    #Stop training when a monitored quantity has stopped improving. https://keras.io/callbacks/#earlystopping
    csv_logger = CSVLogger('Armillotta/history/CONV_Q-QA/log.csv', append=True, separator=';')
    callbacks_list = [checkpoint,csv_logger]


    #LAYER EMBEDDING-BiLSTM FOR ANSWER
    QA_input = Input(batch_shape=(None,QA_train_padded.shape[1],), name='A_input')
    QA = Embedding(len(vocab),output_dim=50, weights=[embedding_matrix],name="Embedding_A",trainable=True)(QA_input)
    ...
    plot_model(model, to_file='Armillotta/image/Q_QA_Conv2D_MaxPool.png', show_shapes=True, show_layer_names=True)


    history = model.fit([Q_train_padded,QA_train_padded],Y_train, batch_size=25, epochs=200, validation_split=0.1,callbacks = callbacks_list)
